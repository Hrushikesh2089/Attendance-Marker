...
def show_count(frame):
    ...
    if image_count <= nimgs:
        cv2.putText(frame, f'Images Captured: {image_count}/{nimgs}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
    else:
        cv2.putText(frame, f'Image Capture Completed!', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)


def generate_frames():
    global image_count, captured_images, is_capturing
    cap = cv2.VideoCapture(0)
    j = 0  # Frame counter for capturing every 5th frame
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            # If capturing is active, store frames
            if is_capturing:
                if j % 5 == 0 and image_count < nimgs:  # Capture every 5th frame
                    face_img = frame[y:y + h, x:x + w]
                    captured_images.append(face_img)
                    image_count += 1
                j += 1

        # Update the display with image count or completion message
        show_count(frame)
